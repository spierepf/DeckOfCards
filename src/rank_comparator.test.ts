...
test('a rank comparator constructed with a subset of ranks leaves comparisons involving missing ranks undefined', () => {
    expect(new RankComparator([Rank.Nine, Rank.Ten, Rank.Jack, Rank.Queen, Rank.King, Rank.Ace]).compare(Rank.Eight, Rank.Nine)).toBeUndefined()
})

test('two is higher thant ace in king-high ordering', () => {
    expect(RankComparator.kingHigh().compare(Rank.Two, Rank.Ace)).toBeGreaterThan(0)
})
